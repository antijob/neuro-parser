...
from .serializers import MediaIncidentSerializer, IncidentTypeSerializer, ArticleSerializer, SourceSerializer
...
class SourceViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter('order_by', openapi.IN_QUERY, description="Order by field (e.g., 'name', 'created_at')", type=openapi.TYPE_STRING)
        ],
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            try:
                ...
                return Response({'error': f'Invalid order_by value: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('limit', openapi.IN_QUERY, description="Limit the number of results", type=openapi.TYPE_INTEGER),
            openapi.Parameter('order_by', openapi.IN_QUERY, description="Order by field (e.g., 'published_at', 'title')", type=openapi.TYPE_STRING)
        ],
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('limit', openapi.IN_QUERY, description="Limit the number of results", type=openapi.TYPE_INTEGER),
            openapi.Parameter('order_by', openapi.IN_QUERY, description="Order by field (e.g., 'detected_at', 'description')", type=openapi.TYPE_STRING)
        ],
        responses={200: MediaIncidentSerializer(many=True)}
    )
    @action(detail=True, methods=['get'])
...
class ArticleViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'url': openapi.Schema(type=openapi.TYPE_STRING, description='URL of the article'),
            }
        ),
    ...
    def predict(self, request, pk=None):
        try:
            ...
            if not article.is_downloaded:
                return Response({'error': 'Article not fetched'}, status=status.HTTP_400_BAD_REQUEST)
